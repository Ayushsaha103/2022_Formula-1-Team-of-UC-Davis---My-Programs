...
double yk(double V, double Voc0);
...
void EKF(double*** xhatk_1, double*** Pk_1, double I, double Ik_1, double V, double Voc0,
	...
	double yk_, double hk_, double*** Aprime_transpose, double*** Eprime_transpose,
...
double yk(double V, double Voc0) {
	double yk_ = V + Voc0;
	return yk_;		// yk_ is Actual Voltage
}
...
void EKF(double*** xhatk_1, double*** Pk_1, double I, double Ik_1, double V, double Voc0,
	...
	int row, col, index;
	double temp_val = 0.0, temp_val2 = 0.0;
	// CALCULATION FOR xhat
	...
	for (row = 0; row < 2; row++) {
		temp_val += (*sub_mat_1)[row][0] * (*Cprime)[row];
	}
	temp_val += Rk;
	temp_val = pow(temp_val, (-1));

	for (row = 0; row < 2; row++) {
		(*Lk)[row][0] = (*sub_mat_1)[row][0] * temp_val;
		// TODO/ QUESTION: SHOULD I CHANGE Lk TO LONG DOUBLE?
	}

	// CALCULATIONS FOR xhatCorrected
	temp_val = (*xhat)[0][0];
	temp_val2 = (*xhat)[1][0];
	temp_val = yk(V, temp_val2) - hk(100 * temp_val, I, Voc0, R0);
	...
	for (row = 0; row < 2; row++) {
		for (index = 0; index < 2; index++) {
			(*sub_mat_1)[row][index] = (*Lk)[row][0] * (*Cprime_transpose)[index][0];
		}
	}
...
int main() {
	...
	double yk_ = yk(V, Voc0);
	double hk_ = hk(100 * xhat[0][0], I, Voc0, R0);
	...
	while (!feof(fp)) {
		...
		EKF(&xhatk_1, &Pk_1, I, Ik_1, V, Voc0, Rk, &Aprime, &Cprime, &Eprime,
			&Fprime, &fk_, dt, Cbat, Cc, Rc, &Qk1, yk_, hk_,
